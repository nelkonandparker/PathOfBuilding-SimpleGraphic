...
{
	// Check if the line is visible. pos[Y] is in framebuffer pixels, so it must
	// be compared against the pixel-space screen height (VirtualScreenHeight),
	// matching the VirtualScreenWidth() used for the horizontal cull below --
	// not vid.size[1], which is the window height in logical points.
